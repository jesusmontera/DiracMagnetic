...
class GLBlochWidget(QtOpenGL.QGLWidget):    
    ...
    def drawArrows(self):        
        ...
        for i in range (na):        
            ...
            drawVector(pos,vec*self.spacesize/2.,1.0,0.3,color)                    
    def drawAxes(self):        
        color=[1.,1.,1.]
        # X
        pos = [self.spacesize/2, self.spacesize/2+1, -self.spacesize/2]
        self.drawOGLText(pos,"X ",fontsize=14)
        pos =np.array(pos)*-1.
        pos[0]+=1
        pos[1]+=1
        vec=[1,0.,0]
        drawVector(pos,vec,5,0.2,color)
        # Y
        pos = [-self.spacesize/2-1,self.spacesize/2,-self.spacesize/2]
        self.drawOGLText(pos,"Y",fontsize=14)
        pos =np.array(pos)*-1.
        pos[1]+=1.3
        #pos[0]+=1.
        vec=[0,1.0,0]
